...
While we do have forwards models of the JWST PSF, these models are for single exposure images. The JWST images are either single exposure or mosaics [@2014SPIE; @2012SPIE]. Mosaiced images are essentially single exposure detector images averaged together. To account for the rotation of the camera between the capture of images and the wide field of view, there are a number of steps that make applying these forward models to mosaics a non trivial procedure. There is recent work being done to generate hybrid models using WebbPSF models with an empirical correction, but there is yet to be any widely available or open source software to do this [@lin2023hybpsf].  
